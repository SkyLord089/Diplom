...
const mixableElements: MixableElement[] = [
...
const mixPhrases = ['Отличный выбор! 🎯', 'Готовим колбы... ⚗️', 'Интересная комбинация! 🤔', 'Химия в действии! ✨', 'Ожидаем реакцию... 🧪', 'Научный эксперимент! 🔬'];
...
export default function App() {
  ...
  const [lastResult, setLastResult] = useState<{ success: boolean; compound?: Compound } | null>(null);
  ...
  const [dailyProgress, setDailyProgress] = useState({ mix: 0, quiz: 0, study: 0, memory: 0 });
  ...
  useEffect(() => {
    const check = (ach: typeof achievements[0], cond: boolean) => {
  ...
  return (
    <div className="min-h-screen text-slate-800 bg-slate-50 relative overflow-x-hidden">
      ...
      {/* Навигация */}
      <nav className="bg-white/40 backdrop-blur-xl shadow-sm sticky top-0 z-40 border-b border-white/60 relative">
